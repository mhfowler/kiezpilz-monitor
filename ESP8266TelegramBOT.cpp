...
/*
  ...
  */
  ...
  /*
   * 2018/08/01 AppsByDavideV
   * The library has been slightly modified to suit the MKRWIFI1010 board
   */
...
void TelegramBOT::begin(void)	{
    ...
    //client.setInsecure(); // not available in wifinina
}
...
String TelegramBOT::connectToTelegram(String command)  {
    ...
    long now;
    bool avail;
    char server[] = "api.telegram.org";    // use DNS, let's telegram decide
    
    // Connect with api.telegram.org 
    // example: https://api.telegram.org/bot<token>/METHOD_NAME      
    ...
    client.stop();    //it looks like it is needed
    return mess;
}
 ...
 ***************************************************************/
boolean TelegramBOT::getUpdates(String offset)  {
